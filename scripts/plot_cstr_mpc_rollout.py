...
from aiogym.control.baselines import MPCAgent, make_meas
...
def run_episode(seed: int, steps: int = 320, sp_step_at: int = 160):
    ...
    agent = MPCAgent(make_model("cstr"))
    env.reset(seed=seed)
    agent.reset()
...
def plot(out_path: str):
    ...
    series = [
        (ca, None, "Ca", "Ca (MPC)"),
        (temp, t_sp[0], "T (deg C)", "T (MPC)"),
        (cooling, None, "cooling u", "Cooling u (MPC)"),
    ]
    ...
    axes[0].set_title("CSTR MPC Rollout and Plotting", loc="left", fontsize=15, pad=12)
    fig.tight_layout()
    fig.savefig(out_path, dpi=160, bbox_inches="tight")


if __name__ == "__main__":
    output = os.path.join(ROOT, "output", "cstr_mpc_rollout.png")
