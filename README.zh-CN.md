...
| 评估 | 评估主要靠脚本和 helper 串起来。 | `aiogym.evaluation` 负责 benchmark protocol、rollout、metrics、reports、plots、artifact 生成和 suites。 |
...
- `core.py` 定义 benchmark protocol 和 rollout/evaluation 逻辑。
- `metrics/` 放指标计算器。
...
```text
aiogym/
  api.py          # 稳定 Python 用户入口
  cli/            # 只放终端入口
  models/         # 过程模型、kernel、registry、model cards
...
关键原则是：`api.py` 和 `cli/` 是进入系统的门面，不是核心逻辑的存放处。核心行为应该放在 `models/`、`controllers/`、`evaluation/` 或 `rl/` 中。
